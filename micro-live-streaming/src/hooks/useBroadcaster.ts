...
const useBroadcaster = (options: UseBroadcasterOptions) => {
    ...
    useEffect(() => {
        ...
        socket.on('connect', () => {
            ...
            socket.emit('join', {slug: liveSlug}); // 1 etapa
        });
    }, [liveSlug, socket]);
    ...
    useEffect(() => {
        ...
        // criar peer do broadcaster, para a descoberta do "ip"
        // @ts-ignore
        ...
        peerRef.current.on('open', (peer_id) => {
            ...
            // envio meu id_peer para registrar o id do broadcaster
            socket.emit('set-broadcaster', {peer_id, password})
        });

        // Quando alguem se conecta no broadcaster esse evento dispara
        peerRef.current.on('connection', (conn: Peer.DataConnection) => {
            // aqui eu passo minha meu peer e passo o stream de dados
            // streamRef no react garante que eu sempre pegue a info atualizada, sempre o que muda e o current, o objeto mantem imutavel
