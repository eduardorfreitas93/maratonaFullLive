...
export const Chat: React.FC<ChatProps> = (props) => {
    ...
    useEffect(() => {
        ...
        // evento de conexao, assim que o usuario se conectar no socket
        socket.on('connect', () => {
            console.log('chat connected');

            // envento para pegar todas as mensagens
            socket.on(
                'get-messages',
                (data: ChatMessage[]) => setChatMessages(data)
            );

            // pegar toda mensagem nova
            socket.on(
                'new-message',
                (data: ChatMessage) => {
        ...
        // assim que eu sai desse componente ele desconecta no socket
        return () => {
            if (socket.connected) {
                socket.disconnect();
            }
