...
export class LiveSocketService implements OnGatewayInit {
    ...
    // para eu me conectar na sala e descobrir meu peer
    ...
    async onJoin(
        ...
        try {
            ...
            client.join(slug); // criar a sala WS e me vincula a sala
            console.log('join', client.id);

            await redisSet(client.id, slug); // armazenar no redis os dados do id e do slug

            const peer_id = await redisGet(slug); // pega no redis o dado do id_peer do broadcaster
            client.emit('get-broadcaster', {peer_id}); // envio para mim mesmo o id_peer do broadcaster

            const countUsers = this.getUsersConnected(client, slug);
            client.emit('count-users', countUsers); // enviando a mensagem para mim
            client.broadcast.to(slug).emit('count-users', countUsers); // enviando a mensagem para todos
    ...
    async setBroadcaster(
        ...
        try {
            ...
            await redisSet(slug, data.peer_id); // salvar id_peer do broadcaster

            // enviar para todos que estao na salado WS
